-- ============================================================
-- TNV Leadership Selection Portal — Schema (Supabase Auth based)
-- Run this once in the Supabase SQL Editor on a fresh project.
-- ============================================================

-- ─── PROFILES (one row per auth.users, holds role) ───────────
...
-- is_admin() — SECURITY DEFINER so RLS on profiles itself doesn't recurse
...
-- ─── CATEGORIES (optional tags e.g. Panai, DrugFreeTN) ────────
...
-- Coverage" UI, alongside district/constituency/pincode) — same pattern:
...
-- interviewer_id = auth.uid() — it never checked that candidate_id was
...
-- ─── CALL LOGS (every call attempt made to a candidate) ────────
...
-- interviewer_id = auth.uid() — it never checked that candidate_id was in
...
-- date of birth — age is computed at display time from this, never stored
...
-- status — for the "All Candidates" browse tab), on top of (not instead
...
-- already assigned to anyone — this is what makes "Take Interview" work for
...
-- ─── CANDIDATE CATEGORIES (many-to-many) ───────────────────────
...
-- ─── INTERVIEWER COVERAGE (nested category → district → constituency) ──
...
-- claim-unassigned policy that depended on it — an interviewer no longer
...
-- 1. Create the first admin user in Supabase Dashboard → Authentication → Users → Add User
