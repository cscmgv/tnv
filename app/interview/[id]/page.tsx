...
import InterviewWizard, { type ResumeAssessment } from "../InterviewWizard";
...
export default async function AssignedInterviewPage({ params }: { params: Promise<{ id: string }> }) {
  const { id } = await params;
  const { supabase, profile } = await requireProfile("interviewer");

  // RLS scopes this to candidates assigned to this interviewer, created by
  // them, or in one of their districts — if none of that applies, the row
  // just won't come back here (not a 403, a plain not-found).
  ...
  return (
    <InterviewWizard
      ...
      backHref="/interview"
      backLabel="← Back to My Candidates"
    />
  );
}
